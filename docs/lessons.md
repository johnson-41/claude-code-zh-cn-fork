...
- 2026-04-09: 诊断“插件更新后为什么没生效”时，不能只看仓库里的最新 `patch-cli.js`。必须同时检查 `~/.claude/plugins/claude-code-zh-cn-fork/` 里的已安装 payload 是否包含 patch 所需文件，以及 `.patched-version` 的判断条件是否会在“同一 Claude Code 版本、不同插件版本”下重新 patch。
