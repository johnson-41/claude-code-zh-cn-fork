...
function runDoctor(options = {}) {
  ...
  // auto-repatch 状态
  const repatchMarkerFile = path.join(pluginRoot, ".patched-version");
  const lastUpdateStatusFile = path.join(pluginRoot, ".last-update-status");
  if (fs.existsSync(repatchMarkerFile)) {
    const markerContent = fs.readFileSync(repatchMarkerFile, "utf8").trim();
    add("auto-repatch", "自动重 patch", "ok", `标记: ${markerContent}`);
  } else if (fs.existsSync(pluginRoot)) {
    add("auto-repatch", "自动重 patch", "warn", "无 .patched-version 标记，首次会话将自动 patch");
  }

  // 上次更新状态
  if (fs.existsSync(lastUpdateStatusFile)) {
    const statusContent = fs.readFileSync(lastUpdateStatusFile, "utf8").trim();
    const statusOk = statusContent.startsWith("ok") || statusContent.startsWith("noop");
    add("last-update", "上次自动更新", statusOk ? "ok" : "warn", statusContent);
  }
