...
[![npm stable](https://img.shields.io/badge/npm%20stable-2.1.92--2.1.110-green)](./docs/support-matrix.md)
[![macOS installer](https://img.shields.io/badge/macos%20installer-2.1.104%20experimental-yellow)](./docs/support-matrix.md)
...
| 系统 / 通道 | 当前口径 | 已验证窗口 | 说明 |
|------|---------|-----------|------|
| macOS / npm 全局安装 | `stable` | `2.1.92 - 2.1.110` | 启动前 launcher 自修复 + `session-start` 二层兜底 |
| macOS / 官方安装器 | `experimental` | `2.1.104` | 依赖 native repack；不走 npm launcher |
| Linux / npm 全局安装 | `stable` | `2.1.92 - 2.1.110` | 与 npm stable 同口径 |
| Linux / 官方安装器 | `unsupported` | - | 当前不承诺支持 |
| Windows / WSL + npm 全局安装 | 跟随 npm `stable` | `2.1.92 - 2.1.110` | **必须在 WSL 终端内运行** |

> **Windows 用户**：先安装 [WSL](https://learn.microsoft.com/zh-cn/windows/wsl/install)，然后在 WSL 中安装 Claude Code 和本插件。不支持原生 Windows。
>
> **支持边界单一来源**：当前口径以 [docs/support-matrix.md](./docs/support-matrix.md) 为准。该文档由 `scripts/upstream-compat.config.json` + `node scripts/verify-upstream-compat.js --json` 生成，不手写维护。
...
| 安装方式 | 说明 | 当前口径 |
|---------|------|---------|
| `npm install -g @anthropic-ai/claude-code` | npm 全局安装 | `stable` |
| `curl -fsSL https://claude.ai/install.sh \| sh` | 官方安装器 | `experimental`（仅 macOS） |
...
>
> **验证状态**：macOS 官方安装器当前仅把 `2.1.104` 列为 `experimental`。是否扩展版本窗口，以 `docs/support-matrix.md` 为准。
>
...
- ✅ 在支持的安装方式上 patch 硬编码文字（1503 条翻译；当前 stable 代表版本 `2.1.110` 实测 1394 处有效 patch）
...
| UI 文字中文化 | 1503 条翻译，`2.1.110` 实测 1394 处有效 patch | CLI Patch（扫描真实双引号字符串 token 后逐条替换） |
...
It translates 187 spinner verbs, 41 spinner tips, 1503 UI translations, notification messages, and more. The patch now combines safe double-quoted string scanning with targeted structural patch families for high-risk split UI prompts. After Claude Code updates, npm installs self-heal before launch and `session-start` remains as a second fallback. Current support windows are documented in [docs/support-matrix.md](./docs/support-matrix.md).
