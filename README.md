...
[![macOS native](https://img.shields.io/badge/macos%20native-2.1.113--2.1.126%20experimental-yellow)](./docs/support-matrix.md)
[![Linux native](https://img.shields.io/badge/linux%20native-2.1.126%20experimental-yellow)](./docs/support-matrix.md)
...
| macOS / native binary | `experimental` | `2.1.113 - 2.1.126`（不含未发布的 `2.1.115`） | macOS arm64 native 已验证 extract / patch / repack / `--version`；需要 `node-lief`；未验证新版本会安全跳过 CLI Patch |
| Linux / npm 全局安装 | `stable` | `2.1.92 - 2.1.112` | 与 npm stable 同口径 |
| Linux / native binary | `experimental` | `2.1.126` | Linux native binary 已验证 extract / repack / version；需要 `node-lief` |
| Windows / npm 全局安装 (PowerShell) | `stable` | `2.1.92 - 2.1.112` | PowerShell 安装脚本（install.ps1）；适用于旧 npm cli.js 形态，CLI Patch 可用；需 PowerShell 5.1+ |
...
>
> **最新版说明**：Claude Code 从 `2.1.113` 开始，npm 主包切换为 native binary wrapper，不再包含旧的 `cli.js`。本插件当前 stable CLI Patch 支持到 `2.1.112`；macOS arm64 native binary 现在有独立 experimental 通道，已验证 `2.1.113`、`2.1.114`、`2.1.116 - 2.1.123` 的二进制改写链路和 11 个稳定显示面。`2.1.115` 官方未发布；`latest` 不是 stable 承诺，未验证的新版本会跳过 CLI Patch。
...
| Claude Code native binary `2.1.113 - 2.1.123`（macOS arm64，不含未发布的 `2.1.115`） | 当前已验证的 native binary 版本，显示审计 11/11 PASS | `experimental`（需要 `node-lief`） |
...
> **native binary 说明**：官方安装器和新版 npm 包都可能装到 native 二进制，不是旧 npm `cli.js`。本插件的处理方法是：用 `bun-binary-io.js` 提取二进制里的 JS → 复用 `patch-cli.sh` 翻译 → 再写回二进制。macOS arm64 `2.1.110`、`2.1.111`、`2.1.112` 已在临时目录验证通过；`2.1.113`、`2.1.114`、`2.1.116 - 2.1.123` 额外通过 11 个稳定显示面审计。运行时需要 `node-lief`。要最稳，请优先使用 npm pinned 安装方式。
>
...
- ✅ 在 stable 安装方式上 patch 硬编码文字（1653 条翻译；当前 stable 代表版本 `2.1.112` 实测 1517 处有效 patch，显示审计 11/11 PASS）
- ✅ 在 macOS native experimental 已验证版本上 patch 硬编码文字（`2.1.113`、`2.1.114`、`2.1.116 - 2.1.123` 实测 1323-1358 处，显示审计 11/11 PASS）
...
| UI 文字中文化 | 1653 条翻译，`2.1.112` 实测 1517 处有效 patch；macOS native experimental `2.1.113`、`2.1.114`、`2.1.116 - 2.1.123` 实测 1323-1358 处；固定显示面审计均为 11/11 PASS | CLI Patch（扫描真实双引号字符串 token 后逐条替换）+ 显示面审计 |
...
├── cli-translations.json    ← 1653 条 UI 翻译对照表
...
注意：Claude Code 从 `2.1.113` 开始，npm latest 切换为 native binary wrapper，不再包含旧的 `cli.js`。当前 stable CLI Patch 支持到 `2.1.112`。macOS arm64 native binary 走 experimental 通道，已验证 `2.1.113`、`2.1.114`、`2.1.116 - 2.1.123` 的二进制改写链路和 11 个稳定显示面；`2.1.115` 官方未发布，未验证 latest 不代表 stable 支持。
</details>
...
- Bug → 提 [Issue](https://github.com/KongBai1145/claude-code-zh-cn/issues)
...
本项目基于 [taekchef/claude-code-zh-cn](https://github.com/taekchef/claude-code-zh-cn) 进行了以下优化：

### 安装流程优化

| 改进项 | 原项目 | 本项目 |
|--------|--------|--------|
| 安装方式 | 需要 git clone + 手动运行 | 支持 `curl \| bash` 一键安装 |
| 依赖检查 | 基础检查 | 自动检测版本，提示安装 node-lief |
| 安装引导 | 无 | 首次安装交互式向导 |
| 进度提示 | 纯文本 | 彩色输出 + 步骤进度 |
...
| 改进项 | 原项目 | 本项目 |
|--------|--------|--------|
| 函数复用 | 4 个函数在两文件重复 | 抽取到 `plugin/lib/common.sh` |
| session-start | 449 行，3 个职责混在一起 | 拆分为独立模块 |
| 安全性 | 临时文件使用 PID | 改用 `mktemp` |
| 错误处理 | 部分支持 | `--update-only` 模式增强 |

### 版本支持扩展

| 平台 | 原项目支持 | 本项目支持 |
|------|-----------|-----------|
| macOS native | 2.1.113 - 2.1.123 | 2.1.113 - **2.1.126** |
| Linux native | 不支持 | **2.1.126** |
| 最新版本适配 | 滞后 | 持续跟进最新版本 |
...
It translates 187 spinner verbs, 41 spinner tips, 1653 UI translations, notification messages, and more. The patch combines safe string scanning for legacy `cli.js` plus experimental native-binary paths for macOS arm64 (2.1.113–2.1.126) and Linux (2.1.126). On Windows, a PowerShell install script (`install.ps1`) is available for the old npm `cli.js` form (2.1.92–2.1.112); Windows native `.exe` / unverified latest builds are detected and skipped for CLI Patch (Layers 1–3 still active).

This fork adds one-click installation, interactive setup wizard, enhanced dependency management, and extended version support. Based on [taekchef/claude-code-zh-cn](https://github.com/taekchef/claude-code-zh-cn).
